...
from movie_backend.forms import CreateMovieForm
from rest_framework.exceptions import PermissionDenied
from datetime import datetime,date
from movie_backend.models import BookingRegister
...
def login(request):
    ...
    return Response({'id': user.id, 'username': user.username, 'token': token.key,'user_type': user_type}, status=HTTP_200_OK)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def search_movie_by_date(request, date):
    movies = Movie.objects.filter(release_date__lte=date)
...
def accept_booking(request, pk):
    ...
    if serializer.is_valid():
        ...
        from_email = 'arunlal.m@admin.com'
...
razorpay_client = RazorpayClient(auth=("rzp_test_g1W1oK4QpOMyMb", "e9kZaz0EXJm4O9Wf3SLWu5YW"))
    ...
    if request.method == "POST":
        try:
            ...
            response_data = {
                ...
                "razorpay_key": "rzp_test_g1W1oK4QpOMyMb",
