...
function MyBookings() {
  ...
  return (
    <div>
      <Navbar />
      <br />
      <h2 className="text-center mt-3 text-light">Booked Tickets</h2>
      <br />
      {bookings.length === 0 ? (
        <p className="text-center">You haven't booked any tickets yet.</p>
      ) : (
        <ul className="col-md-8 offset-2">
          {bookings.map((booking, index) => (
            <li key={index} className="booking-card">
              ...
              <div className="qr-code qr-code-right  text-center">
                <p>
                  <b className="me">Scan QR Code</b>
                </p>
                <img
